...
公開先は `OurJapan/open-tokyo-world`。既存東京タワーrepositoryの履歴と原本は保全し、固定hashで外部入力として参照します。新しい基盤は公開先の作業branchで実装し、人間がPRをレビューします。
...
建物・道路区間・植栽・地形・設備・看板をfeature/part/modelで管理する。地物IDとモデル版と配置を分離し、add / replace / suppress / materialを共通契約にする。[共通台帳と実装範囲](object-registry.md)を参照。現段階は版固定された組み立て計画のコンパイルと契約検証までで、既存Blender runnerの汎用adapter統合は未完了。元地物を細分化せずに道路の任意区間を切り取ることはしない。
