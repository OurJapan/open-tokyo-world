...
2. 比較成果物のパッケージ化と固定版公開は完了。[data221の地物・親tileset・座標解析](plateau-data221.md)も実測済み。次はこの2ファイルの取得・ハッシュ検査・地物IDを保持した取込CLIと保存後検証を実装する。元の高さと旧表示用の平坦化を分け、WebP表示と広場接続を画像確認する。
